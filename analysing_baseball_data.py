...
##
## Copied code from week 3 project on CSV files
## author: @eshrawan
...
def filter_by_year(statistics, year, yearid):
    ...
    """
    table = []
    for subdict in statistics:
        if int(subdict[yearid]) == year:
            table.append(subdict)
    return table
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    list_of_tuples = list(map(lambda looks: (looks[info["playerid"]],
                            formula(info, looks)), statistics))
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    list_of_funtion = []
    stats = read_csv_as_list_dict(info["masterfile"], info["separator"],
                                  info["quote"])
    for one in top_ids_and_stats:
        for subdict in stats:
            if subdict[info["playerid"]] == one[0]:
                list_of_funtion.append('{0:.3f} --- {1} {2}'.format(one[1],
                subdict[info['firstname']], subdict[info['lastname']]))
    return list_of_funtion
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    initial_list = read_csv_as_list_dict(info["battingfile"], info["separator"],
    info["quote"])
    initial_list_by_year = filter_by_year(initial_list, year, info["yearid"])
    top_playerid = top_player_ids(info, initial_list_by_year, formula, numplayers)
    top_player = lookup_player_names(info, top_playerid)
    return top_player
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    finaldict = {}
    for subdict in statistics:
        if subdict[playerid] not in finaldict:
            finaldict[subdict[playerid]] = {var: int(subdict[var]) for var in
            fields}
            finaldict[subdict[playerid]][playerid] = subdict[playerid]
        else:
            finaldict[subdict[playerid]] ={var: (int(finaldict[subdict[playerid]]
            [var]) \
            + int(subdict[var])) for var in fields }
            finaldict[subdict[playerid]][playerid] = subdict[playerid]
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    Inputs:
      ...
      numplayers  - Number of top players to return
    """
    dict_info = read_csv_as_list_dict(info["battingfile"], info["separator"],
    info["quote"])
    aggregate_value = aggregate_by_player_id(dict_info, info["playerid"],
    info["battingfields"])
    val_dict = aggregate_value.values()
    top_playerid = top_player_ids(info, val_dict, formula, numplayers)
    final_value = lookup_player_names(info, top_playerid)
    return final_value
